...
public class PaymentUseCaseTest {
    ...
    void checkPaymentStatus() throws Exception {
        ...
        when(paymentProcessGatewayInterface.checkPaymentStatus(any())).thenReturn("new status");
        when(paymentProcessGatewayInterface.savePayment(any())).thenReturn(payment);

        assertDoesNotThrow(() -> {
            assertNotNull(paymentUseCase.checkPaymentStatus(payment.getNumberLunch()));
        });
    }
    ...
    void processPayment_shouldThrowPaymentExceptionWhenNotFoundLunch() {
        ...
        assertThrows(PaymentException.class, () -> paymentUseCase.processPayment(payment), "Pedido invalido para pagamento: Número de lanche não existe no banco");
    }
    ...
    void processPayment_shouldThrowPaymentExceptionWhenPaymentAlreadyHasProcessed() {
        ...
        assertThrows(PaymentException.class, () -> paymentUseCase.processPayment(payment), "Pagamento já foi processado com sucesso");
    }
    ...
    void processPayment_shouldThrowPaymentExceptionWhenPaymentStillInProgress() {
        ...
        assertThrows(PaymentException.class, () -> paymentUseCase.processPayment(payment), "Processando pagamento");
    }
    ...
    void processPayment_shouldThrowPaymentExceptionWhenCpfDoesNotExists() {
        ...
        assertThrows(PaymentException.class, () -> paymentUseCase.processPayment(payment), "CPF precisa existir no banco da lanchonete");
    }
    ...
    void processPayment_shouldThrowIOExceptionWhenProcessPaymentFail() throws IOException, InterruptedException {
        ...
        assertThrows(RuntimeException.class, () -> paymentUseCase.processPayment(payment));
    }
    ...
    void processPayment_shouldProcessPaymentWithoutError() throws IOException, InterruptedException {
        ...
        assertDoesNotThrow(() -> {
            assertNotNull(paymentUseCase.processPayment(payment));
        });
    }
}